...
from src.services.indexing_service import (
    IndexingService,  # Import IndexingService
)
...
mock_logs: List[LogEntry] = [
    ...
    LogEntry(
        ...
        details={"disk": "/dev/sda1", "available": "10%"},
    ),
    LogEntry(
        timestamp=datetime.now(),
        level="ERROR",
...
async def trigger_ingestion(
    ...
    try:
        ...
        logger.info(
            f"Admin '{admin_user['username']}' triggered ingestion for source IDs: {request.source_ids}"
        )
...
async def create_source(
    request: CreateSourceRequest, admin_user: Any = Depends(get_current_admin_user)
):
    """
    ...
    logger.info(
        f"Admin '{admin_user['username']}' configured new source: {new_source.name} ({new_source.type}) at {new_source.path}"
    )

    return {
        "message": "Documentation source configured successfully.",
...
async def update_source(
    ...
    for i, source in enumerate(mock_sources):
        if source.id == source_id:
            ...
            logger.info(
                f"Admin '{admin_user['username']}' updated source {source_id}: {mock_sources[i]}"
            )
            return mock_sources[i]

    logger.warning(
        f"Admin '{admin_user['username']}' attempted to update non-existent source: {source_id}"
    )
    raise HTTPException(
...
async def delete_source(
    source_id: str, admin_user: Any = Depends(get_current_admin_user)
):
    """
    ...
    if len(mock_sources) < initial_len:
        ...
        logger.warning(
            f"Admin '{admin_user['username']}' attempted to delete non-existent source: {source_id}"
        )
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Source with ID {source_id} not found.",
...
async def trigger_reindex(
    ...
    sources_for_reindex = [
        {"file_path": s.path, "metadata": {"id": s.id, "name": s.name, "type": s.type}}
...
async def get_logs(
    ...
    """
    logger.info(
        f"Admin '{admin_user['username']}' requested logs with level {level} and limit {limit}."
    )
...
async def update_rag_config(
    config_update: RAGConfigUpdate, admin_user: Any = Depends(get_current_admin_user)
):
    """
    ...
    if updated_fields:
        logger.info(
            f"Admin '{admin_user['username']}' updated RAG config fields: {', '.join(updated_fields)}. New config: {mock_rag_config}"
        )
        return {
            "message": "RAG configuration updated successfully.",
            "updated_fields": updated_fields,
            "new_config": mock_rag_config,
        }
    else:
        logger.info(
            f"Admin '{admin_user['username']}' provided no RAG configuration fields for update."
        )
